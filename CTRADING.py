...
'''
Currency_Dataframe_Dict.json now represents every currency in the Binance system with a usable dataframe
Can now test strategies on these dataframes
'''


def individual_dict(crypto):
    '''
    Pulls individual historical data for this specific crypto, returns a dataframe
    '''
    f = open('Currency_Dataframe_Dict.json')
    data = json.load(f)
    KEYS = [x for x in data.keys()]
    d = data[crypto]

    converted = json.loads(d)
    return (pd.DataFrame.from_dict(converted))


print(individual_dict('XRPBTC'))
...
# for curr in set(create_sorted_currency_dict().keys()):
    
#     date = get_earliest(curr)    
    
#     B = Backtest(curr,date,'06/10/2022',[(65,75), (15,25), (15,25)], -.005)
#     if B.failed_df == False:
        
#         currencies.append(curr)
#         dataframes.append(B.data.to_json())       
#         print(curr)    

# currency_df_dict = dict(zip(currencies,dataframes))
# with open('Currency_Dataframe_Dict.json', 'w') as fp:
#     json.dump(currency_df_dict, fp)
